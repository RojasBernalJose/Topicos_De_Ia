...
import io  # Para leer datos de texto
...
# --- Carga de datos ---
csv_data = """Humedad (%),Cultivo,Elevación (m),Salinidad (dS/m),Temperatura (°C),Latitud,Longitud
...
print(f"Datos cargados exitosamente. Total: {len(df)} filas.")
print(df.head())

# --- Definición de columnas ---
COLS_GEO = ['Latitud', 'Longitud']                # entradas KNN
Y_NUM = ['Elevación (m)', 'Salinidad (dS/m)']     # salidas KNN (regresión)
Y_CAT = 'Cultivo'                                 # salida KNN (clasificación)

COLS = ['Latitud', 'Longitud', 'Elevación (m)', 'Salinidad (dS/m)']
COLS_CAT = ['Cultivo']
...
knn_r.fit(X_geo, y_num)
print("KNN de regresión entrenado.")

knn_c = KNeighborsClassifier(n_neighbors=3)
knn_c.fit(X_geo, y_cat)
print("KNN de clasificación entrenado.")

# --- Pipeline de normalización ---
prep = ColumnTransformer(
    transformers=[('cat', OneHotEncoder(), COLS_CAT)],
...
X_pipe = df[COLS_CAT + COLS]
Xn = pipe.fit_transform(X_pipe)
print(f"Pipeline ajustado. Shape transformado: {Xn.shape}")

# --- Proyección de un sensor a 7D normalizado ---
def proj_sensor(lat, lon, knn_r, knn_c, pipe):
    """
    Proyecta (lat, lon) al espacio 7D normalizado usando
    predicciones KNN (Elev, Sal, Cultivo) + pipeline.
    """
    df1 = pd.DataFrame([[lat, lon]], columns=COLS_GEO)

    pr_num = knn_r.predict(df1)[0]
    elev = pr_num[0]
    sal = pr_num[1]

    cult = knn_c.predict(df1)[0]
    ...
    v = pipe.transform(df_s)[0]
    return v

# --- Prueba rápida ---
lat_ej = 25.55
lon_ej = -108.48
v_pr = proj_sensor(lat_ej, lon_ej, knn_r, knn_c, pipe)
print(f"\nPrueba proyección ({lat_ej}, {lon_ej}):")
print(v_pr)

# --- Constantes de optimización ---
K = 5                      # número de sensores
D = K * 2                  # dimensiones (lat, lon) por sensor

# --- Costo para una partícula ---
def costo_part(x):
    """
    Costo total para una partícula (k sensores en vector [lat1,lon1,...]).
    """
    try:
        coords = x.reshape(K, 2)

        vs = [proj_sensor(lat, lon, knn_r, knn_c, pipe) for lat, lon in coords]
        vs = np.array(vs)

        # Distancias cuadráticas a cada punto original normalizado
        M = cdist(Xn, vs, 'sqeuclidean')
        dmin2 = M.min(axis=1)

        return dmin2.sum()
    except Exception as e:
        print(f"Error en partícula: {e}")
        return np.inf

def costo_lote(Xb):
    """
    Envoltorio requerido por PySwarms: recibe lote de partículas y
    retorna costos por partícula.
    """
    n = Xb.shape[0]
    return np.array([costo_part(Xb[i]) for i in range(n)])

print(f"\nFunción de costo lista para {K} sensores.")

# --- Límites geográficos ---
lat_min, lat_max = df['Latitud'].min(), df['Latitud'].max()
lon_min, lon_max = df['Longitud'].min(), df['Longitud'].max()

min_b = [lat_min, lon_min] * K
max_b = [lat_max, lon_max] * K
bounds = (np.array(min_b), np.array(max_b))
print("Límites geográficos definidos.")
...
print("\nIniciando PSO...")
best_cost, best_pos = opt.optimize(costo_lote, iters=100)
print("Optimización terminada.")

# --- Resultados ---
print(f"\nMejor costo (SSE): {best_cost:.4f}")

coords_opt = best_pos.reshape(K, 2)
print(f"\nUbicaciones óptimas para {K} sensores (Lat, Lon):")
for i, (lat, lon) in enumerate(coords_opt, start=1):
...
# Paleta de colores 
paleta = {'Maíz': '#F0C242', 'Chile': '#C12C2C', 'Tomate': '#5D9C3A'}
# Formas por cultivo
formas = {'Maíz': 'o', 'Chile': 's', 'Tomate': 'D'}

# 1) Graficar puntos originales por cultivo con formas distintas
for cult, marca in formas.items():
    df_sub = df[df['Cultivo'] == cult]
    plt.scatter(
        df_sub['Longitud'],
        df_sub['Latitud'],
        s=80,
...
# 2) Sensores optimizados 
opt_lons = coords_opt[:, 1]
opt_lats = coords_opt[:, 0]
plt.scatter(
    opt_lons,
    opt_lats,
    marker='^',         # triángulo
    ...
    edgecolor='white',
    linewidth=1.5,
    label='Sensores Optimizados (PSO)'
)

# 3) Etiquetas y estilo
plt.title('Ubicación Óptima de Sensores (PSO) en Guasave', fontsize=16)
plt.xlabel('Longitud', fontsize=12)
plt.ylabel('Latitud', fontsize=12)
plt.grid(True)
plt.legend(title='Leyenda', loc='lower right', frameon=True)
plt.show()
